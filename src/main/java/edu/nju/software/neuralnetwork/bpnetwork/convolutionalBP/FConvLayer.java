 ...
 */
public class FConvLayer extends Layer{
    ...
    public FConvLayer(int filter_num, int[] filter_size, int last_filter_num) {
    ...
    @Override
    public void backpropagation(DMatrix[] errors) {
        for (int i = 0; i < filter_num; i++) {
            DMatrix delta = null;
            try {
        ...
        DMatrix[] prev_errors = new DMatrix[last_filter_num];
        for (int i = 0; i < last_filter_num; i++) {
